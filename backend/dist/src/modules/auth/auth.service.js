...
const src_1 = require("../../../libs/dtos/src");
...
const src_2 = require("../../../libs/types/src");
const src_3 = require("../../../libs/dtos/src");
...
let AuthService = class AuthService {
    ...
    async registerUser(body) {
        ...
        await this.userService.addTransactions(src_2.TransactionActionTypes.ACCOUNTCREATION, newUser, this.entity, user_account_transaction_entity_1.UserAccountTransaction, user_transaction_actiontypes_entity_1.UserTransactionActionTypes);
        const { productName, backendUrl } = this.configService.get(src_2.ConfigEnum.SERVER);
    ...
    async socialLogin(body) {
        ...
        if (socialProvider == src_2.SocialProviderEnum.GOOGLE) {
            return this.googleClient.validate(body);
        }
        if (socialProvider == src_2.SocialProviderEnum.FACEBOOK) {
    ...
    async registerAdminMod(body) {
        ...
        const { productName, backendUrl } = this.configService.get(src_2.ConfigEnum.SERVER);
        const token = this.jwt.sign({ id: (await newUser).id });
        await this.mailService.sendVerificationMail(user.email, {
            authLoginLink: `${backendUrl}/auth/verify?token=${token}`,
    ...
    async login(body) {
        ...
        if (!user || (user.role === src_2.UserRoleEnum.CUSTOMER && user.disable === true)) {
            throw new common_1.HttpException('User not found', common_1.HttpStatus.NOT_FOUND);
        }
        if (!user || (user.role === src_2.UserRoleEnum.CUSTOMER && user.status === src_2.UserStatusEnum.UNVERIFIED)) {
            throw new common_1.HttpException('User needs Verification', common_1.HttpStatus.NOT_FOUND);
        }
        if (user.role === src_2.UserRoleEnum.MODERATOR && user.status === src_2.UserStatusEnum.BLOCK) {
        ...
        return new src_1.AuthorizeResponseDto(user, this.helper.generateToken(user));
    }
    async updateUserAccess(updateAccessDto) {
        ...
        let message = '';
        if (updateAccessDto.role === src_2.UserRoleEnum.MODERATOR) {
            message = 'User Role Successfully updated to Moderator!';
        }
        if (updateAccessDto.role === src_2.UserRoleEnum.ADMIN) {
            message = 'User Role Successfully updated to Admin!';
        }
        if (updateAccessDto.role === src_2.UserRoleEnum.CUSTOMER) {
            message = 'User Role Successfully updated to Member!';
        }
        return new src_3.GlobalResponseDto(message);
    }
    async updateUserStatus(updateStatusDto) {
        ...
        await this.repository.save(user);
        const message = updateStatusDto.status === src_2.UserStatusEnum.VERIFIED
    ...
    async updateModeratorStatus(updateStatusDto) {
        ...
        if (user.role === src_2.UserRoleEnum.MODERATOR) {
            user.setStatus(updateStatusDto.status);
            await this.repository.save(user);
            if (updateStatusDto.status === src_2.UserStatusEnum.BLOCK) {
    ...
    async getAllUsers() {
        const users = await this.repository.find({
            where: { disable: false, role: (0, typeorm_1.In)([src_2.UserRoleEnum.FAKE, src_2.UserRoleEnum.CUSTOMER]) },
        });
        users.forEach((user) => {
            delete user.password;
        });
    ...
    async getUserByMail(email) {
        const user = await this.repository.findOne({
            where: { disable: false, role: src_2.UserRoleEnum.CUSTOMER, email },
        });
        return user;
    }
    async getUserByVerificationToken(token) {
        const secret = this.configService.get(src_2.ConfigEnum.JWT_TOKEN).secret;
    ...
    async verifyEmail(token, res) {
        ...
        try {
            ...
            if (user.profile.isEmailVerified === src_2.UserStatusEnum.UNVERIFIED) {
                let currentCoins = await this.userService.getCurrentCoinsFromDB(user);
                currentCoins += await this.userService.getActionTypeCostFromDB(src_2.TransactionActionTypes.EMAILCONFIRMED);
                await this.userService.transaction(user, currentCoins, src_2.TransactionActionTypes.EMAILCONFIRMED);
            }
            data.isEmailVerified = src_2.UserStatusEnum.VERIFIED;
...
and inform you about voucher codes and much more. Your ZIZLE support team.`;
    ...
    async verifyProfile(id) {
        try {
            ...
            const data = await this.profile.findOneBy({ id: pid });
            data.isProfileVerified = src_2.CustomerProfileEnum.VERIFIED;
    ...
    async forget(email) {
        ...
        return new src_3.GlobalResponseDto(`Please check email. Reset Password link sent to ${user.email}, If not wait for a few minutes`);
    }
    async resetPassword(token, { newPassword }) {
        try {
            ...
            if (user.role === src_2.UserRoleEnum.CUSTOMER && user.status === src_2.UserStatusEnum.UNVERIFIED) {
        ...
        return new src_3.GlobalResponseDto('Password reset successfully!');
    }
    async onlineUsers(token) {
        try {
            ...
            const users = await this.repository.find({
                where: {
                    role: (0, typeorm_1.In)([src_2.UserRoleEnum.CUSTOMER, src_2.UserRoleEnum.MODERATOR]),
                    online: true,
                    disable: false,
                },
            });
            const obj = {
                moderators: users.filter((user) => user.role === src_2.UserRoleEnum.MODERATOR),
                customers: users.filter((user) => user.role === src_2.UserRoleEnum.CUSTOMER),
            };
            return obj;
        }
        catch (error) {
    ...
    async getUserReplies(token, params) {
        try {
            const { modId, duration } = params;
            const tokenValue = token.split(' ')[1];
            const decoded = await this.helper.decode(tokenValue);
            const user = await this.helper.validateUser(decoded);
            if (!user)
            ...
            return await Promise.all(mods.map(async (mod) => {
                return await this.chatService.statsModGotRepliesFromUser({ modId: mod.id, duration });
            }));
        }
        catch (error) {
    ...
    async getMods(token) {
        try {
            ...
            return await this.repository.find({
                where: {
                    role: src_2.UserRoleEnum.MODERATOR,
                    disable: false,
                },
            });
        }
    ...
    async getModSendMessageStats(token, params) {
        try {
            ...
            const mods = await this.getMods(token);
            return await Promise.all(mods.map(async (mod) => {
                return await this.chatService.statsModGotSentToUser({ modId: mod.id, duration });
            }));
        }
        catch (error) {
            throw new common_1.HttpException('Could not fetch data', common_1.HttpStatus.BAD_REQUEST);
    ...
    async createBulkMessages(token, fakeUserIds, customerUserIds, message, file) {
        try {
            ...
            for (const customerUser of customerUsers) {
                const fakeUserIndex = (Chats === null || Chats === void 0 ? void 0 : Chats.length) % (fakeUsers === null || fakeUsers === void 0 ? void 0 : fakeUsers.length);
            ...
            throw new common_1.HttpException('could not send bulk Messages', common_1.HttpStatus.BAD_REQUEST);
        }
    }
    ...
    async deleteModById(token, id) {
        ...
        return new src_3.GlobalResponseDto('User Deleted successfully');
    }
    ...
    async deleteBonusCodeById(token, id) {
        ...
        if (res)
            return new src_3.GlobalResponseDto('Bonus Code Deleted Successfully');
        else {
            return new src_3.GlobalResponseDto('Bonus Code Could not be deleted');
        }
    }
    ...
    async deleteScheduledMessage(token, id) {
        try {
            ...
            if (res)
                return new src_3.GlobalResponseDto('Schedule Message Deleted Successfully');
            else {
                return new src_3.GlobalResponseDto('Schedule Message deleted');
            }
        }
        catch (error) {
    ...
    async sendSpamMessages(token, body, file) {
        try {
            ...
            const customerUsers = await this.repository.find({ where: { id: (0, typeorm_1.In)(customerUserIds) } });
            ...
            return new src_3.GlobalResponseDto('Messages Sent Successfully');
        }
        catch (error) {
            throw new common_1.HttpException('Messages could not be sent', common_1.HttpStatus.BAD_REQUEST);
        }
    }
    ...
    async detectAndReturnNewUser() {
        ...
        const usersWithoutAdminSender = await this.repository
            ...
            .where('user.role = :role', { role: src_2.UserRoleEnum.CUSTOMER })
    ...
    async getAdminUserIds() {
        const adminUsers = await this.repository.findBy({ role: src_2.UserRoleEnum.ADMIN });
    ...
    async sendMessageToUsers() {
        ...
        for (const scheduledMessage of scheduledMessages) {
            ...
            const queryUsers = await this.userService.findAll(null, params, true);
            const fakeUsers = (_a = queryUsers === null || queryUsers === void 0 ? void 0 : queryUsers.data) === null || _a === void 0 ? void 0 : _a.filter((user) => user.role === src_2.UserRoleEnum.FAKE);
    ...
    async newVerifiedCustomers() {
        ...
        const newVerifiedUsers = await this.repository
            .createQueryBuilder('user')
            .where('user.status = :status', { status: src_2.UserStatusEnum.VERIFIED })
