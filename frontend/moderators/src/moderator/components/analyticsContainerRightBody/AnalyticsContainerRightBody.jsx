...
import { createNotesAction, deleteNoteAction, getCustomerNoteAction, modCustomerListAction, updateNoteAction } from '../../../store/slices/moderatorApi/actions';
...
const AnalyticsContainerRightBody = () => {
  ...
  const { userList, customerNote: { data: notesByCustomerId }, realCustomerId, fakeUserId } = useSelector((state) => state.moderatorApi)
  ...
  const distinctOnlineUsers = duplicateOnlineUsers ? [...new Map(duplicateOnlineUsers.map((m) => [m.id, m])).values()] : [];
      ...
      <div className={styled.profileActiveUser}>
        {location?.pathname === "/visit-fake-users" &&
          <>
            ...
            />
            <label> <u><h3>Select all users</h3></u></label>
          </>
        }
            ...
            <>
              {users?.filter((user) => user?.role === 'CUSTOMER').map((user) => (
                <button className={styled.imageButton} key={user.id} onClick={() => {
                  ...
                  {realCustomerBulkIds.some(realUser => realUser.id === user.id) || realCustomerId === user.id ? (
                    <>
                      <img src={tickedUser} alt="loading" />
                      <span className={styled.user_name}>
                        {user.userName}  &nbsp;
                        {distinctOnlineUsers?.some(onlineUser => onlineUser.id === user?.id) && (
                    ...
                    <>
                      <img src={user?.profile?.avatarUrl ? user?.profile?.avatarUrl : tempUserImg} alt='loading' />
                      {userCount?.length > 0 && userCount.map((count) =>
      ...
      <div className={styled.profileActiveUserDetails}>
        <ul>
          ...
          <li>
            ...
            <h3>
              Interested In: <span>{selectedUser?.interestedGender ? selectedUser?.interestedGender : "N/A"}</span>
            </h3>
          </li>
          <li>
            <FontAwesomeIcon icon={faCalendar} />
            <h3>
              Dob: <span>{selectedUser?.profile?.dateOfBirth ? calculateAge(selectedUser?.profile?.dateOfBirth) : "N/A"}</span>
            </h3>
          </li>
          ...
          <li>
            <FontAwesomeIcon icon={faCalendar} />
            <h3>
              Relationship Status: <span>{selectedUser?.profile?.relationshipStatus ? selectedUser?.profile?.relationshipStatus : "N/A"}</span>
            </h3>
          </li>
          <li>
            <FontAwesomeIcon icon={faCalendar} />
            <h3>
              Smoke: <span>{selectedUser?.profile?.smoker ? selectedUser?.profile?.smoker : "N/A"}</span>
            </h3>
          </li>
          <li>
            <FontAwesomeIcon icon={faCalendar} />
          ...
          <li>
            <FontAwesomeIcon icon={faCalendar} />
            <h3>
              Children: <span>{selectedUser?.profile?.children ? selectedUser?.profile?.children : "N/A"}</span>
            </h3>
          </li>
        </ul>
      </div>
