...
export const userLoginAction = createAsyncThunk(
  ...
  async (user, { rejectWithValue }) => {
    try {
      ...
      toast.error('User need verification', {
        position: toast.POSITION.TOP_RIGHT,
      });
...
export const subscriptionEditAccessAction = createAsyncThunk(
  ...
  async (payload, { rejectWithValue }) => {
    try {
      ...
      const updateSubsAccess = await axios.put(`${BASE_URL}/subscriptions/updateSubscription/${payload.id}`, payload, config);
...
export const subsDeleteAccessAction = createAsyncThunk(
  ...
  async (payload, { rejectWithValue }) => {
    try {
      const deleteSubs = await axios.delete(`${BASE_URL}/subscriptions/deleteSubscription/${payload.id}`, config);
      toast.success('Subs Customer Deleted successfully!', {
        position: toast.POSITION.TOP_RIGHT,
      });
      return deleteSubs;
    } catch (error) {
      toast.error('SubsCustomer Delete Unsuccessfully!', {
        position: toast.POSITION.TOP_RIGHT,
      });
      return rejectWithValue(error.message);
    }
