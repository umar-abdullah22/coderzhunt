...
let ModService = class ModService {
    async getUsers(token) {
        ...
        const users = await this.repository.find({
            where: {
                role: (0, typeorm_1.In)([src_1.UserRoleEnum.FAKE, src_1.UserRoleEnum.CUSTOMER]),
            },
        });
    ...
    async getUnseenMessageCount(token) {
        ...
        if (user.role !== src_1.UserRoleEnum.MODERATOR)
            throw new common_1.HttpException('User must be Moderator!', common_1.HttpStatus.NOT_FOUND);
        const customerRoleUsers = await this.repository.find({ where: { role: src_1.UserRoleEnum.CUSTOMER } });
        const fakeRoleUsers = await this.repository.find({ where: { role: src_1.UserRoleEnum.FAKE } });
    ...
    async getAvailableUsers(token) {
        ...
        return await this.repository.find({
            where: {
                role: src_1.UserRoleEnum.CUSTOMER,
                online: true,
            },
        });
    }
...
__decorate([
    (0, typeorm_2.InjectRepository)(user_fallout_entity_1.FallOutUsers),
    __metadata("design:type", typeorm_3.Repository)
], ModService.prototype, "fallOutUsersRepo", void 0);
__decorate([
