...
export class UserService {
  ...
  public async findAll(token?: string, params?: GetUsersQueryParamsDto, schedule?: boolean) {
    ...
    const { page, pageSize, gender, nickname, online, startAge, endAge, distanceInMiles, newUsers, fsk, postalCode } =
      params;
  ...
  async saveMultiplePhotosToDB(photosPaths, photos, user: User) {
  ...
  public async contactSupport(token: string, body: ContactSupportDto): Promise<GlobalResponseDto> {
    const tokenValue = token.split(' ')[1];
    const decoded = await this.helper.decode(tokenValue as string); // verify access token and get user from db
