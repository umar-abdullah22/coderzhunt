...
and inform you about voucher codes and much more. Your ZIZLE support team.`;
  ...
  public async getUserReplies(token: string, params?: GetModsStatsQueryParamsDto) {
    try {
  ...
  async createBulkMessages(token, fakeUserIds: string[], customerUserIds: string[], message: string, file) {
    try {
      ...
      for (const customerUser of customerUsers) {
        const fakeUserIndex = Chats?.length % fakeUsers?.length; // Get the index of the next fakeUser to visit this customerUser
      ...
      throw new HttpException('could not send bulk Messages', HttpStatus.BAD_REQUEST);
    }
  }
  ...
  public async sendSpamMessages(token: string, body: SpamMessagesDto, file): Promise<GlobalResponseDto> {
    try {
      ...
      throw new HttpException('Messages could not be sent', HttpStatus.BAD_REQUEST);
    }
  }
