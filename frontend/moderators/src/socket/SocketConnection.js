...
export const ConnectionProvider = (props) => {
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.on('receiveMessage', (data) => {
        if(data.moderatorIds && data.moderatorIds?.length && data?.moderatorIds?.includes(currentUser?.id)){
  ...
  useEffect(() => {
    ...
    socket?.emit('unseenMessageCount', {userId:currentSelectedUser?.id, moderatorId:currentUser?.id});
  ...
  const acknowledgeUnseenMessageCount = (fakerId,realCustomerId) => {
    if (socket) {
      socket?.emit('acknowledge', { currentUser:fakerId }, { currentSelectedUser:realCustomerId },{currentModerator:currentUser?.id})
    }
  };
